\documentclass[a4paper, 11pt]{article}

\input{includes.tex}

\titleformat*{\section}{\color{verdeEJ}\Large\fontfamily{phv}\selectfont\bfseries}
\titleformat*{\subsection}{\color{verdeEJ}\large\fontfamily{phv}\selectfont\bfseries}
\titleformat*{\subsubsection}{\color{verdeEJ}\large\fontfamily{phv}\selectfont}

\begin{document}
\fontfamily{phv}\selectfont

\input{capa.tex} %%ALTERE A CAPA AQUI%%

\tableofcontents
\newpage
%%%%%%%%%%% EDITE A PARTIR DAQUI %%%%%%%%%% 
\section{Introdução}
\input{Seções/Introducao.tex}
\newpage

\section{Análise de Requisitos}
\input{Seções/Analise de Requisitos}
\newpage


\section{Diagrama de Blocos}
\input{Seções/diagrama_de_blocos.tex}
\newpage

\section{Protótipos de Interface}
\input{Seções/Prototipos de Interface.tex}
\newpage

%\section{Modelagem do Circuito}
%\input{Seções/Modelagem de Circuito.tex}
%\newpage

\section{Descrição de Hardware}
\input{Seções/Descricao de Hardware.tex}
\newpage

\section{Fluxograma de Software}
\input{Seções/Fluxograma_de_Software.tex}
\newpage

\section{Descrição de Software}
\input{Seções/Descricao_de_Software.tex}
\newpage

%\section{Descricao de alto nível}
%\input{Seções/Descricao de Alto nivel.tex}
%\newpage

%\section{Descrição de Firmware}
%\input{Seções/Descricao de Firmware.tex}
%\newpage

\section{Manual de Uso Básico}
\input{Seções/Manual de uso.tex}
\newpage

\section{Bill of Materials}
\input{Seções/BOM.tex}
\end{document}
